// ==UserScript==
// @name         Smartplus ASM GADAR - Chrome + Firefox Violentmonkey Compatible v2.92
// @namespace    smartplus-auto-asm-v221
// @version      2.88.0
// @description  v2.90: fix pemilihan kelompok BB racikan pada batas 5 kg; BB 10 kg -> 5–10 kg, BB 15 kg -> 10–15 kg. Tetap kompatibel Chrome/Firefox + Violentmonkey.
// @author       OpenAI
...
(function () {
  ...
  async function runCombinedComplaintRecipes(complaintKeys, weight, options = {}) {
    ...
    for (let complaintIndex = 0; complaintIndex < complaintKeys.length; complaintIndex++) {
      ...
      updateRecipeProgress("KOMBINASI RESEP: keluhan", complaintIndex, complaintKeys.length, COMPLAINT_RECIPE_MAP[complaintKey]?.label || complaintKey);
      const resolved = await resolveComplaintRecipe(complaintKey, weight, options);
    ...
    if (errors.length) {
      toast(`PAKET RESEP: ${success} obat kombinasi berhasil masuk, ${plan.skipped.length} duplikat dilewati, ${errors.length} bagian gagal.`);
    } else {
      toast(`PAKET RESEP: ${success} obat kombinasi berhasil masuk, ${plan.skipped.length} duplikat dilewati. Tetap sebagai draft.`);
    }

    return { details, seenSet, success, skipped: plan.skipped.length, errors };
  }
  ...
  async function selectAutocomplete(input, query) {
    ...
    const q = norm(query);
    let option = options.find((el) => norm(
      el.innerText || el.textContent || ""
    ) === q);

    if (!option) {
      option = options.find((el) => norm(
        el.innerText || el.textContent || ""
      ).includes(q));
    }
  ...
  function ensureStyle() {
    ...
    style.textContent = `
      ...
      #${MENU_ID}.sp-package-modal{
        ...
        width:620px!important; max-width:calc(100vw - 24px)!important;
      ...
      .sp-preview-card-compact{width:min(980px,calc(100vw - 24px))!important;max-height:calc(100vh - 24px)!important;overflow:hidden!important;background:#fff!important;color:#222!important;border-radius:12px!important;box-shadow:0 10px 36px rgba(0,0,0,.4)!important;padding:12px!important;box-sizing:border-box!important;font:13px Arial,sans-serif!important;}
      .sp-preview-card-compact .sp-preview-grid-compact{display:grid!important;grid-template-columns:1fr 1fr!important;gap:8px!important;}
      .sp-preview-card-compact .sp-preview-box-compact{background:#f8f9fa!important;border:1px solid #e4e8ec!important;border-radius:7px!important;padding:7px!important;min-height:0!important;box-sizing:border-box!important;}
      ...
      .sp-preview-card-compact .sp-preview-list-compact{font-size:10px!important;line-height:1.18!important;max-height:155px!important;overflow:hidden!important;}
      ...
      @media(max-width:700px){.sp-preview-card-compact{max-height:calc(100vh - 14px)!important;padding:9px!important}.sp-preview-card-compact .sp-preview-grid-compact{grid-template-columns:1fr!important}.sp-preview-card-compact .sp-preview-list-compact{max-height:120px!important}}
      #${STATUS_ID}{
    ...
    async function runPackageRecipe() {
      ...
      try {
        ...
        await new Promise((resolve)=>{
          overlay.querySelector('[data-preview-cancel]').addEventListener('click',()=>{ overlay.remove(); resolve(false); });
          overlay.querySelector('[data-preview-continue]').addEventListener('click',()=>{ overlay.remove(); resolve(true); });
        }).then(async proceed=>{
